...
public class GoFishGame extends CardGame {
    ...
    public ArrayList<Card> playerHand() {
        try {
            ...
            System.out.println ("Here is your cards " + playerHand.toString () + ". GoodLuck " + goFishPlayer.getName () + " !!!");
            return playerHand;
        } catch (Exception e) {
    ...
    public Boolean checkComputerHandForPlayerRequestedCard(String rank) {
        ...
        for (int i = 0; i < getComputerHand ().size (); i++) {
            if (rank.equals (getComputerHand ().get (i).getRank ().getSymbol ())) checkOutcome = true;
        }
        return checkOutcome;
    }
    ...
    public void askComputerHandForACard(String rank) {
        ...
        while (checkComputerHandForPlayerRequestedCard (rank)) {
            ...
            System.out.println ("The computer has " + rank + ".The card" + " is added to your hand.\n Insert the next card you want: ");
    ...
    public Boolean checkPlayerHasRequestedCardRank(String rank) {
        ...
        for (int i = 0; i < getPlayerHand ().size (); i++) {
            if (rank.equals (getPlayerHand ().get (i).getRank ().getSymbol ())) checkOutcome = true;
        }
        return checkOutcome;
    }
    ...
    public Map<String, Long> playerHandDisplay() {
        ...
        Map<String, Long> result = getPlayerHand ().stream ().map (card -> card.getRank ().getSymbol ()).collect (Collectors.groupingBy (Function.identity (), Collectors.counting ()));
    ...
    public Card checkPlayerHandForBook() {
        ...
        for (int i = 0; i < getPlayerHand ().size (); i++) {
            ...
            for (int j = i + 1; !found && j < getPlayerHand ().size (); j++) {
                if (getPlayerHand ().get (i).equals (getPlayerHand ().get (j))) found = true;
                count++;
    ...
    public void removeBookedCard() {
        try {
            ...
            for (int i = 0; i < getPlayerHand ().size (); i++) {
                card = getPlayerHand ().get (i);
                if (card.getRank ().getSymbol ().equals (cardTobeRemoved)) tobeRemovedList.add (card);
            }
    ...
    public Card checkComputerHandForBook() {
        ...
        for (int i = 0; i < getComputerHand ().size (); i++) {
            ...
            for (int j = i + 1; !found && j < getComputerHand ().size (); j++) {
                if (getComputerHand ().get (i).equals (getComputerHand ().get (j))) found = true;
                count++;
...
//    }
    ...
    public void decideWiner() {
        ...
        if (countBooksInComputerHand () > countBooksInPlayerHand ()) System.out.println (" The computer won");
        ...
        else System.out.println ("Sorry the score is not yet available. Please check again");
    }
